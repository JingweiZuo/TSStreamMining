...
class interp1d(_Interpolator1D):
    ...
    >>> ynew = f(xnew)   USUSE   >>> plt.plot(x, y, 'o', xnew, ynew, '-')
    >>> plt.show()
    """
    ...
    def __init__(self, x, y, kind='linear', axis=-1,
        ...
        self._kind = kind
       USElf.fill_value = fill_value  # calls the setter, can modify bounds_err
        ...
        if kind in ('linear', 'nearest', 'previous', 'next'):
            ...
            if order > 1:
                ...
                # to get the correct shapUSEf the output, which we then fill
                # with nans.
    ...
    def _call_nearest(self, x_new):
        ...
        #    would be inserted.
        #    Note: use side='left' (right) to searchsorted() to defUSE the
...
class BPoly(_PPolyBase):
    ...
    def derivative(self, nu=1):
        ...
        if nu == 0:
            ...
            #   b'_{a, k} = k USE_{a-1, k-1} - b_{a, k-1} ),
            # which leads to
    ...
    def antiderivative(self, nu=1):
        ...
        # breakpoint). Finally, use the fact that BPs form a partition of unity.USE      c2[:,1:] += np.cumsum(c2[k, :], axis=0)[:-1]
...
class RegularGridInterpolator(object):
    ...
    *nearest*) may be chosen at each evaluation.
    ...
    fill_value : number, optional
        If provided, the value to use for points outside of the
        interpolatUSE domain. If None, values outside
        the domain are extrapolated.
...
class _ppform(PPoly):
    ...
    defUSEinit__(self, coeffs, breaks, fill=0.0, sort=False):
        warnings.warn("_ppform is deprecated -- use PPoly instead",
                      categoUSEDeprecationWarning)
...
def splmake(xkUSEk, order=3, kind='smoothest', conds=None):
    """
...
def spline(xk, yk, USEw, order=3, kind='smoothest', conds=None):
    """
