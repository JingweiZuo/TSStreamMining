...
def data_ConceptDrft():
    global drift, loss_batch, avg_loss, t_stamp, thread
    if drift == True:
        drift_num = 1
    else:
        #drift_num = float('nan')
    ...
    if thread == None:
        ...
        thread.start()
    #y = 2 ** x
    print("Drift2: " + str(drift_num) + "t_stamp:" +str(t_stamp) + " avg_loss:" +str(avg_loss) + " loss_batch:" +str(loss_batch))
    return jsonify(t_stamp=[t_stamp], drift_num=[drift_num], avg_loss=[avg_loss], loss_batch=[loss_batch], label_avg_loss = ['avg_loss'], label_loss_batch=['loss_TSmicroBatch'], label_concept_drift=['concept drift area'])

#TS data in new Window
#Question: How to read the window size and change the TS data shown in the GUI?
@account_api.route('/TS_window/', methods=['POST'])
def data_TSWindow():
    global inputTSBatch, TS_set
    #How to return TS list data? -> as an object list?
    #print("inputTSBatch is: " + str(inputTSBatch))
    return jsonify(inputTSBatch=[inputTSBatch], TS_set=[TS_set])
